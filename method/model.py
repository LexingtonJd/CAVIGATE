...
class DLDKD(nn.Module):
    ...
    '''2025/2/28-start'''
    def group_soft_aggregate_loss_singlefn(self, x, label_dict,
                                           alpha=10.0,
                                           margin=0.2,
                                           focal_p=1.1,
                                           z_clip=50.0,
                                           eps=1e-12):
        """
        单函数版本的 Group-level soft-aggregate 损失（推远同类）。
        Args:
            x: (bs, d) 特征张量（会先归一化）
            label_dict: value 为范围，例如 [start, ..., end] 或 (start, end)
            alpha: 控制 exp 的峭度（越大越敏感于超过 margin 的项）
            margin: 希望同类相似度不超过的阈值 m
            focal_p: 如果 >0，会对超过 margin 的偏差 (s - m) ** focal_p 作为权重（可选）
            z_clip: 为避免 exp overflow，把 alpha*(s-m) 上限截断为 z_clip
            eps: 数值稳定小量
        Returns:
            标量损失（平均到 batch）
        """

        def _build_pos_mask(bs, label_dict, device):
            """构建同标签（exclude self）的掩码 N: (bs, bs)"""
            N = torch.zeros((bs, bs), device=device)
            for _, label in label_dict.items():
                start, end = int(label[0]), int(label[-1])
                N[start:end + 1, start:end + 1] = 1.0
            N.fill_diagonal_(0.0)
            return N
        ...
        # 正样本 mask（同标签且非自身）
        N = _build_pos_mask(bs, label_dict, device=device)  # (bs, bs)

        # 只关注超过 margin 的部分（我们要惩罚 s > margin）
        shift = cos - margin
        pos_shift = torch.clamp(shift, min=0.0)  # (bs, bs)

        # 可选 focal 权重：把较大违规（s-m 更大）放大
        if focal_p and focal_p > 0.0:
            weight = pos_shift.pow(focal_p)
        else:
            weight = torch.ones_like(pos_shift)

        # z = alpha * pos_shift；为了稳定性限制上界
        z = alpha * pos_shift
        z = torch.clamp(z, max=z_clip)

        # 仅对正样本位置计算 exp(z) * weight
        exp_term = torch.exp(z) * weight * N  # (bs, bs)

        # per-anchor sum over positives
        sum_exp = exp_term.sum(dim=1)  # (bs,)

        # loss per anchor: log(1 + sum_exp)  （log1p 更稳定）
        loss_per_anchor = torch.log1p(sum_exp + eps)  # (bs,)

        # normalize by num of positives per anchor to avoid组不平衡主导
        pos_count = N.sum(dim=1).clamp(min=1.0)  # (bs,)
        loss_per_anchor = loss_per_anchor / pos_count
